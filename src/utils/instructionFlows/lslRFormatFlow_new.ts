...
/**
 * R-Format LSL Instruction Flow: LSL X1, X2, X3
 * Implements the exact workflow specification for R-Type left shift instructions
 * 
 * Stage 3: Execute (EX) - Perform left shift operation (X2 << X3)
...
// Stage 3: Execute (EX) - Perform left shift operation
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    // Transform D_Rn_Idx=X2 to D_Rn_Val=0x000F at RegFile
    {
      type: 'transform',
      sourceCircleIds: ['D_Rn_Idx_X2'],
      targetComponent: 'RegFile',
      timing: 200,
      targetDataValue: 'D_Rn_Val=0x000F',
    ...
    {
      type: 'move',
      sourceCircleIds: ['D_Rn_Val_0x000F'],
    ...
    // 4. ALU operation - ALUControl signals left shift
    {
      type: 'move',
    ...
    {
      type: 'move',
      sourceCircleIds: ['alu_lsl_operation'],
      targetComponent: 'ALUMain',
      timing: 1100,
      wirePath: ALUCONTROL_TO_ALUMAIN_SIGNAL_PATH
    },
    
    // Merge at ALUMain: D_Rn_Val and D_Rm_Val consumed, creating D_Result=0x00F0
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val_0x000F', 'D_Rm_Val_4', 'alu_lsl_operation'],
      targetComponent: 'ALUMain',
      timing: 1300,
      targetDataValue: 'D_Result=0x00F0',
      targetDataType: 'alu_result'
    }
  ],
  finalCircles: ['D_Result_0x00F0'],
...
const memoryStage: StageDataFlow = {
  ...
  duration: 1000,
  initialCircles: ['D_Result_0x00F0'],
  operations: [
    ...
    {
      type: 'move',
      sourceCircleIds: ['D_Result_0x00F0'],
...
const writeBackStage: StageDataFlow = {
  ...
  duration: 2000,
  initialCircles: ['D_Result_0x00F0', 'D_Rd_Idx_X1'],
  operations: [
    ...
    {
      type: 'merge',
      sourceCircleIds: ['D_Result_0x00F0', 'D_Rd_Idx_X1', 'C_RegWrite_1'],
      targetComponent: 'RegFile',
      timing: 800,
...
/**
 * Complete R-Format LSL instruction flow
 * Stages: IF (universal) -> ID (universal) -> EX -> MEM -> WB -> PC Update
 * 
 * Example: LSL X1, X2, X3
 * - Perform left shift operation: X2 << X3 = 0x000F << 4 = 0x00F0
 * - No memory access needed
 * - Write result 0x00F0 to register X1
 * - Update PC to PC+4
 */
export const LSL_R_FORMAT_FLOW = createInstructionFlow([
  executeStage,
  memoryStage,
  writeBackStage,
  pcUpdateStage
