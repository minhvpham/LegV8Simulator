 ...
 * Implements the exact workflow specification for R-Type logical XOR instructions
 * 
 * Stage 3: Execute (EX) - Perform XOR operation (X2 ^ X3)
...
// Stage 3: Execute (EX) - Perform XOR operation
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    // Merge at ALUMain: D_Rn_Val and D_Rm_Val consumed, creating D_Result=0xFF00
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val_0x0F0F', 'D_Rm_Val_0xF0F0', 'alu_xor_operation'],
      targetComponent: 'ALUMain',
      timing: 1300,
...
/**
 * Complete R-Format EOR instruction flow
 * Stages: IF (universal) -> ID (universal) -> EX -> MEM -> WB -> PC Update
 * 
 * Example: EOR X1, X2, X3
 * - Perform XOR operation: X2 ^ X3 = 0x0F0F ^ 0xF0F0 = 0xFF00
 * - No memory access needed
 * - Write result 0xFF00 to register X1
 * - Update PC to PC+4
 */
export const EOR_R_FORMAT_FLOW = createInstructionFlow([
  executeStage,
  memoryStage,
  writeBackStage,
  pcUpdateStage
