...
/**
 * I-Format ORRI Instruction Flow: ORRI X1, X2, #0x0F0F
 * Implements the exact workflow specification for I-Type logical OR immediate instructions
 * 
 * Stage 3: Execute (EX) - Perform OR operation (X2 | immediate)
...
// Stage 3: Execute (EX) - Perform OR operation with immediate
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    // Merge at ALUMain: D_Rn_Val and D_SignExt_Imm consumed, creating D_Result=0xFFFF
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val_0xF0F0', 'D_SignExt_Imm_0x0F0F', 'alu_or_operation'],
      targetComponent: 'ALUMain',
      timing: 1300,
...
/**
 * Complete I-Format ORRI instruction flow
 * Stages: IF (universal) -> ID (universal) -> EX -> MEM -> WB -> PC Update
 * 
 * Example: ORRI X1, X2, #0x0F0F
 * - Perform OR operation: X2 | immediate = 0xF0F0 | 0x0F0F = 0xFFFF
 * - No memory access needed
 * - Write result 0xFFFF to register X1
 * - Update PC to PC+4
 */
export const ORRI_I_FORMAT_FLOW = createInstructionFlow([
  executeStage,
  memoryStage,
  writeBackStage,
  pcUpdateStage
