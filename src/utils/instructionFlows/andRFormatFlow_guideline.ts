...
/**
 * R-Format AND Instruction Flow: AND X1, X2, X3
 * Following the EXACT workflow specification from the guideline (same as ADD/SUB except for ALU operation)
 */
...
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    {
      type: 'merge',
      sourceCircleIds: ['C_ALUSrc=0', 'D_Rm_Val=0x0F0F'],
      targetComponent: 'MuxReadRegData',
      timing: 800,
      resultData: 'D_Rm_Val=0x0F0F',
    ...
    // 6. ALU Operation (AND: 0xFF00 & 0x0F0F = 0x0F00)
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val=0xFF00', 'D_Rm_Val=0x0F0F'],
      targetComponent: 'ALUMain',
      timing: 1000,
      resultData: 'D_ALU_Result=0x0F00'
    }
  ],
  finalCircles: ['D_ALU_Result=0x0F00', 'D_Write_Addr_Idx', 'C_RegWrite=1', 'C_MemRead=0', 'C_MemWrite=0', 'C_MemToReg=0', 'C_UncondBranch=0', 'C_ZeroBranch=0'],
...
const memoryStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=0x0F00', 'C_MemRead=0', 'C_MemWrite=0'],
  operations: [
    // Signal Propagation (Bypass) - ALU result travels to two destinations
    {
      type: 'split',
      sourceCircleIds: ['D_ALU_Result=0x0F00'],
      targetComponent: 'ALUMain',
      timing: 0,
      splitResults: [
        {
          newValue: 'D_ALU_Result=0x0F00',
          newType: 'data_signal',
          targetComponent: 'DataMem',
  ...
  finalCircles: ['D_ALU_Result=0x0F00', 'C_MemToReg=0', 'C_RegWrite=1'],
...
const writeBackStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=0x0F00', 'D_Write_Addr_Idx', 'C_MemToReg=0', 'C_RegWrite=1'],
  operations: [
    // Multiplexer Selection
    {
      type: 'merge',
      sourceCircleIds: ['C_MemToReg=0', 'D_ALU_Result=0x0F00'],
      targetComponent: 'MuxReadMemData',
      timing: 0,
      resultData: 'D_ALU_Result=0x0F00',
    ...
    {
      type: 'merge',
      sourceCircleIds: ['C_RegWrite=1', 'D_ALU_Result=0x0F00', 'D_Write_Addr_Idx'],
      targetComponent: 'RegFile',
      timing: 500,
...
// Complete AND instruction flow
export const AND_R_FORMAT_FLOW: StageDataFlow[] = [
  UNIVERSAL_IF_STAGE,
  UNIVERSAL_ID_STAGE,
  executeStage,
  memoryStage,
