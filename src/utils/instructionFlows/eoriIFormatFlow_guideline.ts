...
/**
 * I-Format EORI Instruction Flow: EORI X1, X2, #0xFF
 * Following the EXACT workflow specification from the guideline (same as ADDI except for ALU operation)
 */
...
const executeStage: StageDataFlow = {
  ...
  duration: 3000,
  initialCircles: ['D_Opcode', 'D_Rn_Idx', 'D_Write_Addr_Idx', 'D_Imm'],
  operations: [
    ...
    // 2. ALU Operation - Register read
    {
      type: 'transform',
      sourceCircleIds: ['D_Rn_Idx'],
      targetComponent: 'RegFile',
      timing: 500,
      resultData: 'D_Rn_Val=0xFFFF',
    ...
    // 3. Sign extend immediate
    {
      ...
      timing: 600,
      resultData: 'D_SignExt_Imm=0xFF00',
      wirePath: SIGNEXTEND_TO_MUXREADREG_PATH
    },

    // 4. ALU operand selection through MuxReadRegData
    {
      type: 'merge',
      sourceCircleIds: ['C_ALUSrc=1', 'D_SignExt_Imm=0xFF00'],
      targetComponent: 'MuxReadRegData',
      timing: 700,
      resultData: 'D_SignExt_Imm=0xFF00',
    ...
    // 5. ALU Operation (EORI: 0xFFFF ^ 0xFF00 = 0x00FF)
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val=0xFFFF', 'D_SignExt_Imm=0xFF00'],
      targetComponent: 'ALUMain',
      timing: 800,
      resultData: 'D_ALU_Result=0x00FF'
    }
  ],
  finalCircles: ['D_ALU_Result=0x00FF', 'D_Write_Addr_Idx', 'C_RegWrite=1', 'C_MemRead=0', 'C_MemWrite=0', 'C_MemToReg=0', 'C_UncondBranch=0', 'C_ZeroBranch=0'],
...
const memoryStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=0x00FF', 'C_MemRead=0', 'C_MemWrite=0'],
  operations: [
    // Signal Propagation (Bypass) - ALU result travels to two destinations
    {
      type: 'split',
      sourceCircleIds: ['D_ALU_Result=0x00FF'],
      targetComponent: 'ALUMain',
      timing: 0,
      splitResults: [
        {
          newValue: 'D_ALU_Result=0x00FF',
          newType: 'data_signal',
          targetComponent: 'DataMem',
  ...
  finalCircles: ['D_ALU_Result=0x00FF', 'C_MemToReg=0', 'C_RegWrite=1'],
...
const writeBackStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=0x00FF', 'D_Write_Addr_Idx', 'C_MemToReg=0', 'C_RegWrite=1'],
  operations: [
    // Multiplexer Selection
    {
      type: 'merge',
      sourceCircleIds: ['C_MemToReg=0', 'D_ALU_Result=0x00FF'],
      targetComponent: 'MuxReadMemData',
      timing: 0,
      resultData: 'D_ALU_Result=0x00FF',
    ...
    {
      type: 'merge',
      sourceCircleIds: ['C_RegWrite=1', 'D_ALU_Result=0x00FF', 'D_Write_Addr_Idx'],
      targetComponent: 'RegFile',
      timing: 500,
...
// Complete EORI instruction flow
export const EORI_I_FORMAT_FLOW: StageDataFlow[] = [
  UNIVERSAL_IF_STAGE,
  UNIVERSAL_ID_STAGE,
  executeStage,
  memoryStage,
