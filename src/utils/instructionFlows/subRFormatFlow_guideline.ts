 ...
 * Following the EXACT workflow specification from the guideline (identical to ADD except for ALU operation)
 */
...
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    {
      type: 'merge',
      sourceCircleIds: ['C_ALUSrc=0', 'D_Rm_Val=50'],
      targetComponent: 'MuxReadRegData',
      timing: 800,
      resultData: 'D_Rm_Val=50',
    ...
    // 6. ALU Operation (SUB: 100 - 50 = 50)
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val=100', 'D_Rm_Val=50'],
      targetComponent: 'ALUMain',
      timing: 1000,
      resultData: 'D_ALU_Result=50'
    }
  ],
  finalCircles: ['D_ALU_Result=50', 'D_Write_Addr_Idx', 'C_RegWrite=1', 'C_MemRead=0', 'C_MemWrite=0', 'C_MemToReg=0', 'C_UncondBranch=0', 'C_ZeroBranch=0'],
...
const memoryStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=50', 'C_MemRead=0', 'C_MemWrite=0'],
  operations: [
    // Signal Propagation (Bypass) - ALU result travels to two destinations
    {
      type: 'split',
      sourceCircleIds: ['D_ALU_Result=50'],
      targetComponent: 'ALUMain',
      timing: 0,
      splitResults: [
        {
          newValue: 'D_ALU_Result=50',
          newType: 'data_signal',
          targetComponent: 'DataMem',
  ...
  finalCircles: ['D_ALU_Result=50', 'C_MemToReg=0', 'C_RegWrite=1'],
...
const writeBackStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=50', 'D_Write_Addr_Idx', 'C_MemToReg=0', 'C_RegWrite=1'],
  operations: [
    // Multiplexer Selection
    {
      type: 'merge',
      sourceCircleIds: ['C_MemToReg=0', 'D_ALU_Result=50'],
      targetComponent: 'MuxReadMemData',
      timing: 0,
      resultData: 'D_ALU_Result=50',
    ...
    {
      type: 'merge',
      sourceCircleIds: ['C_RegWrite=1', 'D_ALU_Result=50', 'D_Write_Addr_Idx'],
      targetComponent: 'RegFile',
      timing: 500,
...
// PC Update (Non-Branch) - Same as ADD
...
// Complete SUB instruction flow
export const SUB_R_FORMAT_FLOW: StageDataFlow[] = [
  UNIVERSAL_IF_STAGE,
  UNIVERSAL_ID_STAGE,
  executeStage,
  memoryStage,
