...
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    // 6. ALU Operation (LSR: 0x12340 >> 4 = 0x1234)
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val=0x12340', 'D_Rm_Val=4'],
      targetComponent: 'ALUMain',
      timing: 1000,
      resultData: 'D_ALU_Result=0x1234'
    }
  ],
  finalCircles: ['D_ALU_Result=0x1234', 'D_Write_Addr_Idx', 'C_RegWrite=1', 'C_MemRead=0', 'C_MemWrite=0', 'C_MemToReg=0', 'C_UncondBranch=0', 'C_ZeroBranch=0'],
...
const memoryStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=0x1234', 'C_MemRead=0', 'C_MemWrite=0'],
  operations: [
    // Signal Propagation (Bypass) - ALU result travels to two destinations
    {
      type: 'split',
      sourceCircleIds: ['D_ALU_Result=0x1234'],
      targetComponent: 'ALUMain',
      timing: 0,
      splitResults: [
        {
          newValue: 'D_ALU_Result=0x1234',
          newType: 'data_signal',
          targetComponent: 'DataMem',
  ...
  finalCircles: ['D_ALU_Result=0x1234', 'C_MemToReg=0', 'C_RegWrite=1'],
...
const writeBackStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=0x1234', 'D_Write_Addr_Idx', 'C_MemToReg=0', 'C_RegWrite=1'],
  operations: [
    // Multiplexer Selection
    {
      type: 'merge',
      sourceCircleIds: ['C_MemToReg=0', 'D_ALU_Result=0x1234'],
      targetComponent: 'MuxReadMemData',
      timing: 0,
      resultData: 'D_ALU_Result=0x1234',
    ...
    {
      type: 'merge',
      sourceCircleIds: ['C_RegWrite=1', 'D_ALU_Result=0x1234', 'D_Write_Addr_Idx'],
      targetComponent: 'RegFile',
      timing: 500,
...
// Complete LSR instruction flow
export const LSR_R_FORMAT_FLOW: StageDataFlow[] = [
  UNIVERSAL_IF_STAGE,
  UNIVERSAL_ID_STAGE,
  executeStage,
  memoryStage,
