 ...
 * Implements the exact workflow specification for R-Type logical AND instructions
 * 
 * Stage 3: Execute (EX) - Perform AND operation (X2 & X3)
...
// Stage 3: Execute (EX) - Perform AND operation
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    // Merge at ALUMain: D_Rn_Val and D_Rm_Val consumed, creating D_Result=0x0000
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val_0x0F0F', 'D_Rm_Val_0xF0F0', 'alu_and_operation'],
      targetComponent: 'ALUMain',
      timing: 1300,
...
/**
 * Complete R-Format AND instruction flow
 * Stages: IF (universal) -> ID (universal) -> EX -> MEM -> WB -> PC Update
 * 
 * Example: AND X1, X2, X3
 * - Perform AND operation: X2 & X3 = 0x0F0F & 0xF0F0 = 0x0000
 * - No memory access needed
 * - Write result 0x0000 to register X1
 * - Update PC to PC+4
 */
export const AND_R_FORMAT_FLOW = createInstructionFlow([
  executeStage,
  memoryStage,
  writeBackStage,
  pcUpdateStage
