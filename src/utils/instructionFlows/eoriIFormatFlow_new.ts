...
/**
 * I-Format EORI Instruction Flow: EORI X1, X2, #0x0F0F
 * Implements the exact workflow specification for I-Type logical XOR immediate instructions
 * 
 * Stage 3: Execute (EX) - Perform XOR operation (X2 ^ immediate)
...
// Stage 3: Execute (EX) - Perform XOR operation with immediate
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    // 4. ALU operation - ALUControl signals XOR
    {
      type: 'move',
    ...
    {
      type: 'move',
      sourceCircleIds: ['alu_xor_operation'],
      targetComponent: 'ALUMain',
      timing: 1100,
      wirePath: ALUCONTROL_TO_ALUMAIN_SIGNAL_PATH
    },
    
    // Merge at ALUMain: D_Rn_Val and D_SignExt_Imm consumed, creating D_Result=0xFF00
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val_0xF0F0', 'D_SignExt_Imm_0x0F0F', 'alu_xor_operation'],
      targetComponent: 'ALUMain',
      timing: 1300,
      targetDataValue: 'D_Result=0xFF00',
      targetDataType: 'alu_result'
    }
  ],
  finalCircles: ['D_Result_0xFF00'],
...
const memoryStage: StageDataFlow = {
  ...
  duration: 1000,
  initialCircles: ['D_Result_0xFF00'],
  operations: [
    ...
    {
      type: 'move',
      sourceCircleIds: ['D_Result_0xFF00'],
...
const writeBackStage: StageDataFlow = {
  ...
  duration: 2000,
  initialCircles: ['D_Result_0xFF00', 'D_Rd_Idx_X1'],
  operations: [
    ...
    {
      type: 'merge',
      sourceCircleIds: ['D_Result_0xFF00', 'D_Rd_Idx_X1', 'C_RegWrite_1'],
      targetComponent: 'RegFile',
      timing: 800,
...
/**
 * Complete I-Format EORI instruction flow
 * Stages: IF (universal) -> ID (universal) -> EX -> MEM -> WB -> PC Update
 * 
 * Example: EORI X1, X2, #0x0F0F
 * - Perform XOR operation: X2 ^ immediate = 0xF0F0 ^ 0x0F0F = 0xFF00
 * - No memory access needed
 * - Write result 0xFF00 to register X1
 * - Update PC to PC+4
 */
export const EORI_I_FORMAT_FLOW = createInstructionFlow([
  executeStage,
  memoryStage,
  writeBackStage,
  pcUpdateStage
