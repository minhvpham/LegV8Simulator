...
/**
 * R-Format LSR Instruction Flow: LSR X1, X2, X3
 * Implements the exact workflow specification for R-Type right shift instructions
 * 
 * Stage 3: Execute (EX) - Perform right shift operation (X2 >> X3)
...
// Stage 3: Execute (EX) - Perform right shift operation
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    // Transform D_Rn_Idx=X2 to D_Rn_Val=0x00F0 at RegFile
    {
      type: 'transform',
      sourceCircleIds: ['D_Rn_Idx_X2'],
      targetComponent: 'RegFile',
      timing: 200,
      targetDataValue: 'D_Rn_Val=0x00F0',
      targetDataType: 'register_data'
    },
    
    // Transform D_Rm_Idx=X3 to D_Rm_Val=4 at RegFile (shift amount)
    {
      type: 'transform',
      sourceCircleIds: ['D_Rm_Idx_X3'],
      targetComponent: 'RegFile',
      timing: 220,
      targetDataValue: 'D_Rm_Val=4',
      targetDataType: 'register_data'
    },
    ...
    {
      type: 'move',
      sourceCircleIds: ['D_Rn_Val_0x00F0'],
    ...
    // 4. ALU operation - ALUControl signals right shift
    {
      type: 'move',
    ...
    {
      type: 'move',
      sourceCircleIds: ['alu_lsr_operation'],
      targetComponent: 'ALUMain',
      timing: 1100,
      wirePath: ALUCONTROL_TO_ALUMAIN_SIGNAL_PATH
    },
    
    // Merge at ALUMain: D_Rn_Val and D_Rm_Val consumed, creating D_Result=0x000F
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val_0x00F0', 'D_Rm_Val_4', 'alu_lsr_operation'],
      targetComponent: 'ALUMain',
      timing: 1300,
      targetDataValue: 'D_Result=0x000F',
      targetDataType: 'alu_result'
    }
  ],
  finalCircles: ['D_Result_0x000F'],
...
const memoryStage: StageDataFlow = {
  ...
  duration: 1000,
  initialCircles: ['D_Result_0x000F'],
  operations: [
    ...
    {
      type: 'move',
      sourceCircleIds: ['D_Result_0x000F'],
...
const writeBackStage: StageDataFlow = {
  ...
  duration: 2000,
  initialCircles: ['D_Result_0x000F', 'D_Rd_Idx_X1'],
  operations: [
    ...
    {
      type: 'merge',
      sourceCircleIds: ['D_Result_0x000F', 'D_Rd_Idx_X1', 'C_RegWrite_1'],
      targetComponent: 'RegFile',
      timing: 800,
...
/**
 * Complete R-Format LSR instruction flow
 * Stages: IF (universal) -> ID (universal) -> EX -> MEM -> WB -> PC Update
 * 
 * Example: LSR X1, X2, X3
 * - Perform right shift operation: X2 >> X3 = 0x00F0 >> 4 = 0x000F
 * - No memory access needed
 * - Write result 0x000F to register X1
 * - Update PC to PC+4
 */
export const LSR_R_FORMAT_FLOW = createInstructionFlow([
  executeStage,
  memoryStage,
  writeBackStage,
  pcUpdateStage
