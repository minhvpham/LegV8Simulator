...
/**
 * EOR Instruction Data Flow Definition  
 * Traces the flow: EOR Rd, Rn, Rm
 * Example: EOR X1, X2, X3
 * 
 * Similar to ADD/AND/ORR but performs bitwise XOR operation:
 ...
 * - Performs bitwise XOR of two register values
 */

export const EOR_FLOW: StageDataFlow[] = [
  ...
  // Stage 2: Instruction Decode & Register Read
  {
    ...
    finalCircles: ['pc_plus_4', 'rn_addr', 'rm_addr', 'rd_control', 'eor_control'],
    duration: 1500,
    simultaneousFlows: true
  },

  // Stage 3: Execute (ALU Operation - EOR)
  {
    stageName: 'EXECUTE',
    initialCircles: ['rn_addr', 'rm_addr', 'rd_control', 'eor_control'],
    operations: [
      {
        type: 'transform',
        timing: 0,
      ...
      },
      {
        type: 'merge',
        timing: 600,
        sourceCircleIds: ['rn_at_alu', 'rm_at_alu', 'eor_control'],
        targetComponent: 'ALUMain',
        resultData: 'EOR_RESULT'
      }
    ],
    finalCircles: ['eor_result', 'rd_control'],
    duration: 1000,
    simultaneousFlows: true
  },

  // Stage 4: Memory Access (Idle for EOR)
  {
    stageName: 'MEMORY_ACCESS',
    initialCircles: ['eor_result', 'rd_control'],
    operations: [
      {
        type: 'move',
        timing: 0,
        sourceCircleIds: ['eor_result'],
        targetComponent: 'DataMem', // Pass through memory stage
        resultData: 'EOR_RESULT'
      }
    ],
    finalCircles: ['eor_result_bypass', 'rd_control'],
  ...
  // Stage 5: Write Back
  {
    ...
    initialCircles: ['eor_result_bypass', 'rd_control'],
    operations: [
      {
        type: 'move',
        timing: 0,
        sourceCircleIds: ['eor_result_bypass'],
        targetComponent: 'RegFile',
        resultData: 'EOR_RESULT'
      },
      {
        type: 'merge',
        timing: 200,
        sourceCircleIds: ['eor_result_at_reg', 'rd_control'],
        targetComponent: 'RegFile',
