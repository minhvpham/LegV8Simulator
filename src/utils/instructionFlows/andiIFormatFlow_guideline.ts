...
/**
 * I-Format ANDI Instruction Flow: ANDI X1, X2, #0xFF
...
const executeStage: StageDataFlow = {
  ...
  operations: [
    ...
    {
      type: 'merge',
      sourceCircleIds: ['C_ALUSrc=1', 'D_SignExt_Imm=0xFF'],
      targetComponent: 'MuxReadRegData',
      timing: 700,
      resultData: 'D_SignExt_Imm=0xFF',
    ...
    // 5. ALU Operation (ANDI: 0xFFFF & 0xFF = 0xFF)
    {
      type: 'merge',
      sourceCircleIds: ['D_Rn_Val=0xFFFF', 'D_SignExt_Imm=0xFF'],
      targetComponent: 'ALUMain',
      timing: 800,
      resultData: 'D_ALU_Result=0xFF'
    }
  ],
  finalCircles: ['D_ALU_Result=0xFF', 'D_Write_Addr_Idx', 'C_RegWrite=1', 'C_MemRead=0', 'C_MemWrite=0', 'C_MemToReg=0', 'C_UncondBranch=0', 'C_ZeroBranch=0'],
...
const memoryStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=0xFF', 'C_MemRead=0', 'C_MemWrite=0'],
  operations: [
    // Signal Propagation (Bypass) - ALU result travels to two destinations
    {
      type: 'split',
      sourceCircleIds: ['D_ALU_Result=0xFF'],
      targetComponent: 'ALUMain',
      timing: 0,
      splitResults: [
        {
          newValue: 'D_ALU_Result=0xFF',
          newType: 'data_signal',
          targetComponent: 'DataMem',
  ...
  finalCircles: ['D_ALU_Result=0xFF', 'C_MemToReg=0', 'C_RegWrite=1'],
...
const writeBackStage: StageDataFlow = {
  ...
  duration: 1500,
  initialCircles: ['D_ALU_Result=0xFF', 'D_Write_Addr_Idx', 'C_MemToReg=0', 'C_RegWrite=1'],
  operations: [
    // Multiplexer Selection
    {
      type: 'merge',
      sourceCircleIds: ['C_MemToReg=0', 'D_ALU_Result=0xFF'],
      targetComponent: 'MuxReadMemData',
      timing: 0,
      resultData: 'D_ALU_Result=0xFF',
    ...
    {
      type: 'merge',
      sourceCircleIds: ['C_RegWrite=1', 'D_ALU_Result=0xFF', 'D_Write_Addr_Idx'],
      targetComponent: 'RegFile',
      timing: 500,
...
// Complete ANDI instruction flow
export const ANDI_I_FORMAT_FLOW: StageDataFlow[] = [
  UNIVERSAL_IF_STAGE,
  UNIVERSAL_ID_STAGE,
  executeStage,
  memoryStage,
